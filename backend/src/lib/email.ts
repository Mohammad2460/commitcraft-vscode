...
export async function sendApiKeyEmail(email: string, apiKey: string): Promise<void> {
  await resend.emails.send({
    from: 'CommitCraft AI <noreply@applyfast.dev>',
    to: email,
    ...
    html: `
      <div style="font-family: sans-serif; max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        ...
        <p style="color: #555;">Free plan: <strong>5 generations per month</strong>. Check your current usage anytime in VS Code via <code>Ctrl+Shift+P → CommitCraft: Show Usage</code>.</p>
